...
const Following = ({userId}) => {
    ...
    console.log("from following:", userId)
    ...
    const fetchFollowingList = async (userId) => {
        try {
            // Fetch the user's following list from the database
            ...
            if (followingIds) {
                ...
                const followingArray = Object.keys(followingIds);
    
                // Fetch user details for each following user
                const followingDetails = followingArray.map(async followingId => {
                    const userRef = database().ref(`users/${followingId}`);
                    const userSnapshot = await userRef.once('value');
                    return userSnapshot.val();
                });
    
                // Wait for all user details to be fetched
                const followingUsers = await Promise.all(followingDetails);
                console.log("124",followingUsers)
                setFollowing(followingUsers); // Update the state with fetched following list
            } else {
                setFollowing([]);
            }
        } catch (error) {
    ...
    console.log("This is following Data",following)
